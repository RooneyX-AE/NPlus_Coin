#!/usr/bin/env python3
"""
NPlus Coin Miner - Standalone Edition - FIXED REWARD
Pure mining, fixed 50 NPC, balanced difficulty
"""
...
class WalletManager:
    ...
    def get_summary(self) -> str:
        return f"{self.data['balance']:,} NPC | {self.data['blocks_mined']} blocks | {self.data['address'][:16]}..."
...
class NPlusBlockchain:
    ...
    def __init__(self, config_path: Path, logger: logging.Logger, saved_state: dict = None):
        ...
        self.difficulty = min(4, int(self.config.get("difficulty", Config.INITIAL_DIFFICULTY)))
        self.height = 0
    ...
    def get_reward_for_next_block(self) -> int:
        # FIXED: không halving nữa, luôn 50 NPC
...
class NPlusMiner:
    ...
    def initialize(self):
        ...
        self.logger.info("=" * 60)
        self.logger.info("NPLUS COIN MINER - FIXED 50 NPC EDITION")
        self.logger.info("=" * 60)
    ...
    def run(self):
        ...
        self.logger.info(f"Mining started | Difficulty: {self.blockchain.difficulty} | Reward: {self.blockchain.initial_reward} NPC fixed")
        try:
            while self.running:
                ...
                if self.blockchain.add_block(new_block, difficulty_used):
                    ...
                    self.logger.info(f"Block #{new_block.index} | diff={difficulty_used} | nonce={new_block.nonce:,} | +{reward} NPC | balance={new_balance:,} | {block_rate:,.0f} H/s | {new_block.hash[:16]}...")
                now = time.time()
                if now - last_report >= Config.STATUS_REPORT_INTERVAL:
                    elapsed = now - start_time
                    avg_hr = total_hashes / elapsed if elapsed > 0 else 0
                    self.logger.info(f"Status | blocks={session_blocks} | rewards={session_rewards:,} | avg={avg_hr:,.0f} H/s | balance={self.wallet.get_balance():,} | height={self.blockchain.height} | diff={self.blockchain.difficulty}")
                    last_report = now
        finally:
            self.wallet.save()
            self.storage.save(self.blockchain)
